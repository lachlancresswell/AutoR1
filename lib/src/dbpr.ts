...
export enum ControlFlags {
	...
	RELATIVE = 14,
	SWITCH_OFF = 6,
	SWITCH_OFF_CONFIRM = 1286,
	SWITCH_ON = 4102,
	SWITCH_ON_CONFIRM = 5126,
	SWITCH_TOGGLE = 262,
 ...
 */
export const getAllAsObjects = <T>(stmt: SQLjs.Statement, bindObj: number[] | string[] = []) => {
	const rows: T[] = [];
...
export class ProjectFile extends SqlDbFile {
	...
	constructor(db: SQLjs.Database) {
		...
		if (!this.getMasterGroupID()) {
			throw new Error(
				'Project file is not initialised. The project must be opened the project in R1 and have had all initial groups and views created.'
			);
		}
	}
	...
	public createGroup(group: Partial<Group>): number {
		...
		const { Name, ParentId, TargetId, TargetChannel, Type, Flags } = {
			...defaults,
			...group
		};
		...
		const selectAllGroupsStmt = this.db.prepare(
			'SELECT * FROM Groups ORDER BY GroupId DESC LIMIT 1;'
		);
		const rtn = selectAllGroupsStmt.getAsObject({}) as unknown as Group;
	...
	public getMasterGroupID(): number | undefined {
		...
		const rtn = stmt.getAsObject([1, 'Master']) as unknown as { GroupId: number };
		if (!rtn || rtn.GroupId === undefined) {
	...
	public getSourceGroupNameFromID(id: number): string | undefined {
		...
		const rtn = stmt.getAsObject([id]) as unknown as { Name: string };
	...
	public getSourceGroupIDFromName(name: string): number | undefined {
		...
		const rtn = stmt.getAsObject([name]) as unknown as { SourceGroupId: number };
	...
	public getHighestJoinedID(): number | undefined {
		...
		const rtn = stmt.getAsObject({}) as unknown as { JoinedId: number };
	...
	public getHighestGroupID(): number | undefined {
		...
		const rtn = stmt.getAsObject({}) as unknown as { 'max(GroupId)': number };
	...
	public getGroupIdFromName(name: string): number | undefined {
		...
		const rtn = stmt.get([name]) as unknown as number[];
	...
	public getCanIdFromDeviceId(deviceId: number): string | undefined {
		const stmt = this.db.prepare(
			'SELECT RemoteIdSubnet, RemoteIdDevice FROM Devices WHERE DeviceId = ?'
		);
		const rtn = stmt.getAsObject([deviceId]) as unknown as {
			RemoteIdSubnet: number;
			RemoteIdDevice: number;
		};
		if (!rtn || rtn.RemoteIdSubnet === undefined || rtn.RemoteIdDevice === undefined) {
			console.warn(`Could not find device with id ${deviceId}`);
			return undefined;
		}
		return `${rtn.RemoteIdSubnet}.${
			rtn.RemoteIdDevice < 9 ? `0${rtn.RemoteIdDevice}` : rtn.RemoteIdDevice
		}`;
	}
...
export class TemplateFile extends SqlDbFile {
	constructor(db: SQLjs.Database) {
		...
		const templates = stmt.get({}) as unknown as Section[];
	...
	getTemplateControlsByName(tempName: string): Control[] | undefined {
		const section = this.db
			.prepare(`SELECT * FROM 'main'.'Sections' WHERE Name = ?`)
			.getAsObject([tempName]) as unknown as Section;
		...
		const controls = this.db
			.prepare(`SELECT * FROM Controls WHERE JoinedId = ${section.JoinedId} ORDER BY PosX ASC`)
			.run() as unknown as Control[];
