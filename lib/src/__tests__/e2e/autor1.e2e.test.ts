...
// 		const muteButton = projectFile
// 			.getAllControls()
// 			.find(
// 				(control) =>
// 					control.DisplayName === 'Mute' &&
...
// 		// Act
// 		const fallbackGroup = projectFile
// 			.getAllGroups()
// 			.find((group) => group.Name === FALLBACK_GROUP_TITLE);
...
// 		const fallbackButton = projectFile
// 			.getAllControls()
// 			.find(
// 				(control) =>
// 					control.DisplayName === 'Fallback is active' &&
...
// 			// Act
// 			const digitalSyncControl = projectFile.db
// 				.prepare(query)
// 				.all(
// 					meterViewId,
...
// 			const groupRows = digitalSyncControl.map(
// 				(control) => projectFile.db.prepare(groupQuery).all(control.TargetId) as dbpr.Group[]
// 			);
...
// 			// Act
// 			const digitalSyncControl = projectFile.db
// 				.prepare(controlQuery)
// 				.all(
// 					meterViewId,
// 					dbpr.TargetPropertyType.INPUT_DIGITAL_SYNC,
// 					dbpr.TargetTypes.GROUP
// 				) as dbpr.Control[];
// 			const groupRows = digitalSyncControl.map(
// 				(control) => projectFile.db.prepare(groupQuery).all(control.TargetId) as dbpr.Group[]
// 			);
...
// 				const digitalSyncControl = projectFile.db
// 					.prepare(controlQuery)
// 					.all(meterViewId) as dbpr.Control[];
...
// 						meterViewId,
// 						dbpr.TargetPropertyType.INPUT_DIGITAL_SYNC,
// 						dbpr.TargetTypes.DIRECT_ACCESS
// 					) as dbpr.Control[];
...
// 			// Act
// 			const ovlIndicators = projectFile.db
// 				.prepare(controlQuery)
// 				.all(
// 					meterViewId,
// 					dbpr.ControlTypes.LED,
// 					dbpr.TargetPropertyType.CHANNEL_STATUS_REM_HOLD_GR
// 				) as dbpr.Control[];
...
// 					meterViewId,
// 					dbpr.ControlTypes.LED,
// 					dbpr.TargetPropertyType.CHANNEL_STATUS_REM_HOLD_OVL
// 				) as dbpr.Control[];
...
// 			// Act
// 			const { width: meterTempWidth, height: meterTempHeight } =
// 				templateFile.getTemplateWidthHeight('Group LR AP CPL2');

// 			// Assert
// 			controls.forEach((control, index) => {
// 				expect(Math.round(control.PosX)).toBe(
// 					Positions.MAIN_METERS_FRAME_X + index * METER_SPACING_X + index * meterTempWidth
// 				);
// 			});
// 		});
...
// 			// Act
// 			const rtn = projectFile.db
// 				.prepare(controlQuery)
// 				.all(mainViewId, dbpr.ControlTypes.SWITCH) as dbpr.Control[];
...
// 			// Act
// 			const viewEqSwitches = projectFile.db
// 				.prepare(controlQuery)
// 				.all(mainViewId, dbpr.ControlTypes.SWITCH) as dbpr.Control[];
...
// 			// Act
// 			const group1Switch = projectFile.db
// 				.prepare(controlQuery)
// 				.get(mainViewId, dbpr.ControlTypes.SWITCH) as dbpr.Control;
// 			const group1MainControls = projectFile.db
// 				.prepare(groupControlQuery)
// 				.all(group1Switch.JoinedId) as dbpr.Control[];

// 			// Assert
// 			expect(
// 				group1MainControls.find(
// 					(control) => control.TargetProperty === dbpr.TargetPropertyType.CONFIG_LOAD_MATCH_ENABLE
// 				)
// 			).toBeFalsy();
// 		});
...
// 			// Act
// 			const projectTitle = projectFile.db
// 				.prepare(controlQuery)
// 				.get(mainViewId, 'Auto - Main', dbpr.ControlTypes.TEXT) as dbpr.Control;
// 			const templateTitle = templateFile.db
// 				.prepare(`SELECT * FROM Controls WHERE DisplayName = ? and Type = ?`)
// 				.get('Auto - Main', dbpr.ControlTypes.TEXT) as dbpr.Control;
...
// 			const regularRelDelayControls = projectFile.db
// 				.prepare(controlQuery)
// 				.all(
// 					mainViewId,
// 					overviewView?.ViewId,
// 					'rel. Delay',
// 					dbpr.TargetPropertyDigitalChannel.CHANNEL_STATUS_MS_DELAY
// 				) as dbpr.Control[];

// 			const mainViewRelDelayControls = projectFile.db
// 				.prepare(delayControlQuery)
// 				.all(
// 					mainViewId,
// 					dbpr.TargetPropertyDigitalChannel.CHANNEL_STATUS_MS_DELAY,
...
// 			const muteControls = projectFile
// 				.getControlsByViewId(mainViewId)
// 				.filter(
// 					(control) =>
// 						control.Type === dbpr.ControlTypes.SWITCH &&
...
// 					const lastGroupId =
// 						discoveredMuteChannelTargetIDs[discoveredMuteChannelTargetIDs.length - 1];
...
// 			// Act
// 			const meterControls = projectFile
// 				.getControlsByViewId(mainViewId)
// 				.filter(
// 					(control) =>
// 						control.Type === dbpr.ControlTypes.METER &&
// 						control.TargetProperty ===
// 							dbpr.TargetPropertyType.CHANNEL_STATUS_GAIN_REDUCTION_HEADROOM &&
...
// 				expect(control.TargetChannel).toBeGreaterThan(0);
// 				expect(
// 					discoveredMeterChannelTargetIDs.includes({
// 						TargetId: control.TargetId,
// 						TargetChannel: control.TargetChannel
// 					})
// 				).toBeFalsy();
...
// 			// Act
// 			const overviewView = projectFile.getAllRemoteViews().find((view) => view.Name === 'Overview');
// 			const regularCplSwitches = projectFile.db
// 				.prepare(controlQuery)
// 				.all(mainViewId, overviewView!.ViewId, dbpr.ControlTypes.DIGITAL) as dbpr.Control[];

// 			const mainViewCplSwitches = projectFile.db
// 				.prepare(cplControlQuery)
// 				.all(mainViewId, dbpr.ControlTypes.DIGITAL) as dbpr.Control[];
...
// 		// Arrange
// 		const allMainPageDelayControls = projectFile.db
// 			.prepare(`SELECT * FROM Controls WHERE Type = ? AND TargetProperty = ?`)
// 			.all(
// 				dbpr.ControlTypes.DIGITAL,
// 				dbpr.TargetPropertyType.CHANNEL_STATUS_MS_DELAY
// 			) as dbpr.Control[];
// 		expect(allMainPageDelayControls.length).toBeTruthy();

// 		const allSourcePageDelayControls = projectFile.db
// 			.prepare(
// 				`SELECT * FROM Controls WHERE Type = ? AND TargetProperty = ? AND ViewId != ? AND ViewId != ?`
// 			)
// 			.all(
// 				dbpr.ControlTypes.DIGITAL,
// 				dbpr.TargetPropertyType.CHANNEL_STATUS_MS_DELAY,
...
// 			const mainPageControl = allMainPageDelayControls.find(
// 				(control) => control.TargetId === sourceControl.TargetId
// 			);
...
// 			const viewId = (projectFile as any).getViewIdFromName(EQ_WINDOW_TITLE);
// 			const controls = projectFile
// 				.getControlsByViewId(viewId)
// 				.filter((control) => control.Type === dbpr.ControlTypes.EQ);
...
// 		// Act
// 		const cplSwitches = projectFile.db
// 			.prepare(controlQuery)
// 			.all(mainViewId, dbpr.ControlTypes.DIGITAL) as dbpr.Control[];
