...
import { beforeEach, afterEach, describe, expect, it, afterAll, beforeAll } from 'vitest';
...
describe('getTemplateWidthHeight', () => {
	...
	it('should return falsy for a non-existing template', () => {
		...
		expect(() => templateFile.getTemplateWidthHeight(TEMPLATE_NAME)).toThrow(
...
describe('getTemplateControlsFromName', () => {
	...
	it('should throw an error for a non-existing template', () => {
		const TEMPLATE_NAME = `NonExistingTemplate`;
		expect(() => templateFile.getTemplateControlsFromName(TEMPLATE_NAME)).toThrow(
...
describe('createNavButtons', () => {
	...
	it('should create three nav buttons on all views except the AutoR1 Main and Meter views', () => {
		...
		const navButtonTemplateSize = templateFile.getTemplateControlsFromName(
			AutoR1.AutoR1TemplateTitles.NAV_BUTTONS
		).length;
		...
		const views = projectFile
			...
			.filter(
				(v) =>
					v.Name !== AutoR1.MAIN_WINDOW_TITLE &&
					v.Name !== AutoR1.METER_WINDOW_TITLE &&
					v.Name !== AutoR1.EQ_WINDOW_TITLE
			);
		...
		expect(newControls.length).toBe(
			oldControls.length + numControlsOnDefaultViews + numControlsOnAutoR1Views
		);
	});
	...
	it('should move all controls on all views except the AutoR1 crated views down to make space for the nav buttons', () => {
		...
		const oldControls = projectFile
			.getAllControls()!
			.filter((control) => views.find((view) => view.ViewId === control.ViewId));

		// Act
		projectFile.createNavButtons(templateFile);

		// Assert
		const newControls = projectFile
			.getAllControls()!
			.filter((control) => views.find((view) => view.ViewId === control.ViewId));
	...
	it('should should correctly set the TargetId', () => {
		...
		// Assert
		const viewButtons = projectFile
			.getAllControls()!
			.filter(
				(control) => control.Type === ControlTypes.SWITCH && control.TargetType === TargetTypes.VIEW
			);
	...
	it('should NOT create the MAIN VIEW button on the METER VIEW if the MAIN VIEW does not exist', () => {
		...
		// Assert
		const mainViewButton = projectFile
			.getAllControls()!
			.find(
				(control) =>
					control.ViewId === meterViewId &&
	...
	it('should NOT create the EQ VIEW button on the METER VIEW if the EQ VIEW does not exist', () => {
		...
		// Assert
		const eqViewButton = projectFile
			.getAllControls()!
			.find(
				(control) =>
					control.ViewId === meterViewId &&
	...
	it('should NOT create the METER VIEW button on the MAIN VIEW if the METER VIEW does not exist', () => {
		...
		// Assert
		const meterViewButton = projectFile
			.getAllControls()!
			.find(
				(control) =>
					control.ViewId === mainViewId &&
					control.Type === ControlTypes.SWITCH &&
					control.TargetType === TargetTypes.VIEW &&
	...
	it('should create the METER VIEW button on the MAIN VIEW if the METER VIEW exists', () => {
		...
		const meterViewButton = projectFile
			.getAllControls()!
			.find((control) => control.ViewId === mainViewId && control.TargetId === meterViewId);
	...
	it('should create the EQ VIEW button on the MAIN VIEW if the EQ VIEW exists', () => {
		...
		const eqViewButton = projectFile
			.getAllControls()!
			.find((control) => control.ViewId === mainViewId && control.TargetId === eqViewId);
		expect(eqViewButton).toBeTruthy();
	});
...
describe('createMeterView', () => {
	...
	it('adds controls to the view', () => {
		...
		expect(newControlCount).toBe(oldControlCount + 1994);
	});
});
...
describe('createEqView', () => {
	...
	it('correctly assigns TargetId value for EQ controls', () => {
		...
		let controls = projectFile
			.getAllControls()!
			.filter((c) => c.ViewId === viewId)
			.filter((c) => c.Type === ControlTypes.EQ);
...
describe('createMainView', () => {
	...
	it('adds controls to the view', () => {
		projectFile.createMainView(templateFile);
		const controlCount = projectFile.getControlsByViewId(
			(projectFile as any).getMainView()!.ViewId
		)!.length;
...
describe('insertTemplate', () => {
	...
	beforeAll(async () => {
		...
		projectFile.insertTemplate(template, ViewId, posX, posY, {
			DisplayName,
			TargetId,
			TargetChannel,
			Width,
		...
		insertedControls = projectFile
			.getControlsByViewId(ViewId)!
			.filter((c) => c.JoinedId === JoinedId);
	});
	...
	it('should correctly set the TargetChannel of a new control', () => {
		expect(
			insertedControls!.find((control) => control.TargetChannel === TargetChannel)
		).toBeTruthy();
	});
