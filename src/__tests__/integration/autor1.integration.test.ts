...
describe('getSrcGroupType', () => {
    ...
    it('should get source group type for when given a group name', () => {
        expect(projectFile.getSrcGroupType('8 - Point source').SourceGroupType).toBe(2);
        expect(projectFile.getSrcGroupType('8 - Point source').hasSUBsGroup).toBe(false);
        expect(projectFile.getSrcGroupType('8 - Point source').hasTOPsGroup).toBe(false);
        expect(projectFile.getSrcGroupType('8 - Point source').isLRGroup).toBe(false);
    });

    it('should return correct configuration for Unused channels', () => {
        // Unused channels
        expect(projectFile.getSrcGroupType(1).SourceGroupType).toBe(5);
        expect(projectFile.getSrcGroupType(1).hasSUBsGroup).toBe(false);
        expect(projectFile.getSrcGroupType(1).hasTOPsGroup).toBe(false);
        expect(projectFile.getSrcGroupType(1).isLRGroup).toBe(false);
    });

    it('should return correct configuration for Array two way TOPs', () => {
        expect(projectFile.getSrcGroupType(2).SourceGroupType).toBe(1);
        expect(projectFile.getSrcGroupType(2).hasSUBsGroup).toBe(false);
        expect(projectFile.getSrcGroupType(2).hasTOPsGroup).toBe(true);
        expect(projectFile.getSrcGroupType(2).isLRGroup).toBe(true);
    });

    it('should return correct configuration for Array single channel TOPs', () => {
        expect(projectFile.getSrcGroupType(4).SourceGroupType).toBe(1);
        expect(projectFile.getSrcGroupType(4).hasSUBsGroup).toBe(false);
        expect(projectFile.getSrcGroupType(4).hasTOPsGroup).toBe(true);
        expect(projectFile.getSrcGroupType(4).isLRGroup).toBe(true);
    });

    it('should return correct configuration for Point source', () => {
        expect(projectFile.getSrcGroupType(6).SourceGroupType).toBe(2);
        expect(projectFile.getSrcGroupType(6).hasSUBsGroup).toBe(false);
        expect(projectFile.getSrcGroupType(6).hasTOPsGroup).toBe(false);
        expect(projectFile.getSrcGroupType(6).isLRGroup).toBe(false);
    });

    it('should return correct configuration for SUB array LCR', () => {
        // note: SUB array does not have SUBs group
        expect(projectFile.getSrcGroupType(7).SourceGroupType).toBe(3);
        expect(projectFile.getSrcGroupType(7).hasSUBsGroup).toBe(false);
        expect(projectFile.getSrcGroupType(7).hasTOPsGroup).toBe(false);
        expect(projectFile.getSrcGroupType(7).isLRGroup).toBe(false);
    });

    it('should return correct configuration for Array two way subs LR', () => {
        expect(projectFile.getSrcGroupType(8).SourceGroupType).toBe(1);
        expect(projectFile.getSrcGroupType(8).hasSUBsGroup).toBe(true);
        expect(projectFile.getSrcGroupType(8).hasTOPsGroup).toBe(false);
        expect(projectFile.getSrcGroupType(8).isLRGroup).toBe(true);
    });

    it('should return correct configuration for Single channel SUBs LR', () => {
        expect(projectFile.getSrcGroupType(12).SourceGroupType).toBe(1);
        expect(projectFile.getSrcGroupType(12).hasSUBsGroup).toBe(true);
        expect(projectFile.getSrcGroupType(12).hasTOPsGroup).toBe(false);
        expect(projectFile.getSrcGroupType(12).isLRGroup).toBe(true);
    });

    it('should return correct configuration for Array SUBs TOPs LR', () => {
        expect(projectFile.getSrcGroupType(14).SourceGroupType).toBe(1);
        expect(projectFile.getSrcGroupType(14).hasSUBsGroup).toBe(true);
        expect(projectFile.getSrcGroupType(14).hasTOPsGroup).toBe(true);
        expect(projectFile.getSrcGroupType(14).isLRGroup).toBe(true);
    });

    it('should return correct configuration for Array mono TOPs', () => {
        expect(projectFile.getSrcGroupType(16).SourceGroupType).toBe(1);
        expect(projectFile.getSrcGroupType(16).hasSUBsGroup).toBe(false);
        expect(projectFile.getSrcGroupType(16).hasTOPsGroup).toBe(true);
        expect(projectFile.getSrcGroupType(16).isLRGroup).toBe(false);
    });

    it('should return correct configuration for Point source SUB', () => {
        expect(projectFile.getSrcGroupType(18).SourceGroupType).toBe(2);
        expect(projectFile.getSrcGroupType(18).hasSUBsGroup).toBe(false);
        expect(projectFile.getSrcGroupType(18).hasTOPsGroup).toBe(false);
        expect(projectFile.getSrcGroupType(18).isLRGroup).toBe(false);
    });

    it('should return correct configuration for Point source SUBs TOPs', () => {
        expect(projectFile.getSrcGroupType(19).SourceGroupType).toBe(2);
        expect(projectFile.getSrcGroupType(19).hasSUBsGroup).toBe(true);
        expect(projectFile.getSrcGroupType(19).hasTOPsGroup).toBe(true);
        expect(projectFile.getSrcGroupType(19).isLRGroup).toBe(false);
    });

    it('should return correct configuration for Array ground stack mono', () => {
        expect(projectFile.getSrcGroupType(20).SourceGroupType).toBe(1);
        expect(projectFile.getSrcGroupType(20).hasSUBsGroup).toBe(true);
        expect(projectFile.getSrcGroupType(20).hasTOPsGroup).toBe(true);
        expect(projectFile.getSrcGroupType(20).isLRGroup).toBe(false);
    });

    it('should return correct configuration for Group 1', () => {
        expect(projectFile.getSrcGroupType(22).SourceGroupType).toBe(4);
        expect(projectFile.getSrcGroupType(22).hasSUBsGroup).toBe(false);
        expect(projectFile.getSrcGroupType(22).hasTOPsGroup).toBe(false);
        expect(projectFile.getSrcGroupType(22).isLRGroup).toBe(false);
    });
});
